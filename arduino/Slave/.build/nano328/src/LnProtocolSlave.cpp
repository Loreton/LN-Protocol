...
void sendResponse(byte slaveAddress, byte pinNO);
...
void loop() {
    while (true) {
        uint8_t RECEIVED_FLAG = chekForRequest();
        if (RECEIVED_FLAG) {
            delay(500*devADDR);       // porima di rispondere aspetta un tempo che dipenda dal DevAddress
            sendResponse(devADDR, 1);
        }
...
void sendResponse(byte slaveAddress, byte pinNO) {
    ...
    TxMsg [dataLen++] = slaveAddress;                 // Slave Address
